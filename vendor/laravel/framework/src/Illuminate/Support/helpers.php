...
class general_filter_array{
  ...
  function is_match($array) 
  {
  
    if($array['feature_id']== $this->num )
    {
      return true;
    }
    else
...
function upload_img($img_file,$folder_name)
{
                  
    $imgpath = 'assets/img/'.$folder_name;
...
function generateThumbnail($filename, $tx, $ty, $quality = 90)
{
    list($width, $height) = getimagesize($filename);
    $image_p = imagecreatetruecolor($tx, $ty);
    $image = imagecreatefromjpeg($filename);
    imagecopyresampled($image_p, $image, 0, 0, 0, 0, $tx, $ty, $width, $height);
    $name=time()."_".$filename->getClientOriginalName();
    imagejpeg($image_p, 'assets/img/propertythumb/'.$name, $quality);  
    return $name; 
}

function delete_img($img_file,$folder_name)
{
   $imgpath = 'assets/img/'.$folder_name;
   File::makeDirectory($imgpath, $mode = 0777, true, true);
   $imgDestinationPath = $imgpath.'/';
